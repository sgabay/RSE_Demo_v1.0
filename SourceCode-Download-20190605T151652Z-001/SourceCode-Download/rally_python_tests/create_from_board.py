...
def ws_name_match(name):
	...
	if (server_name == "sales"):
		match = re.match('[a-z]*.*[a-z]@broadcom.com[-]\d\d\d\d[-]\d\d[-][A-Z][a-z][a-z]', name)
	else:
...
def getStoriesStateDefined():
	...
	if not collection.errors:
		...
		if ws_name_match(name):
			...
			task_update = {"FormattedID" : story.FormattedID, "Notes" : "Workspace name does not match criteria for creation.  Names should be in the format -- first.last@broadcom.com-YEAR-MO-Mon as in thomas.mcquitty@broadcom.com-2017-08-Aug", "DisplayColor" : "#ff0000"}
			result = rally.post('Story', task_update)
	return
